...
/*
 * Exam 1: Stereoscopic vision
 *
 * This program reads the file in.raw, representing a model in RAW format.
 * Then it generates the res.ppm file that contains a rasterized version of the model in stereoscopic 3D.
 * The position of the camera is defined in line 145 of main.cpp.
 *
 * Authors:
 *   + Carlos Esteban Lopez Rivas
 */
...
// Returns a matrix that represents a specified RAW file
Matrix *matrixFromRaw(char *fileName, vector<int> *lineEndings) {
	...
	while(!file.eof()) {
		...
		// Save line endings to separate faces
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	w = 1000;
	h = 1000;
	float d = 0.2; // Distance between eyes
